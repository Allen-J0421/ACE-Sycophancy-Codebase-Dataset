...
public class FieldStats
{
    ...
    {
        counters.clear();
        IntStream.range(0, field.getDepth())
            .boxed()
            .flatMap(row -> IntStream.range(0, field.getWidth())
                .mapToObj(col -> field.getObjectAt(row, col)))
            .filter(obj -> obj != null)
            .collect(Collectors.groupingBy(Object::getClass, Collectors.counting()))
            .forEach((cls, count) -> {
                Counter c = new Counter(cls.getName());
                c.setCount(cls == Grass.class && simulator.getSteps() != 0
                    ? simulator.getNoOfGrass() : (int) (long) count);
                counters.put(cls, c);
            });
        countsValid = true;
    }
