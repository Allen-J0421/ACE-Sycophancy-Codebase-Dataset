...
public final class MatrixChainMultiplication {
    ...
    private static int costForSplit(
            ...
            ChainRange range,
            int split) {
        int leftSubchainCost = minimumCosts.get(range.leftSubchain(split));
        int rightSubchainCost = minimumCosts.get(range.rightSubchain(split));
    ...
    private static final class ChainRange {
        ...
        private ChainRange leftSubchain(int split) {
            return fromStartAndEnd(start, split);
        }

        private ChainRange rightSubchain(int split) {
            return fromStartAndEnd(split, end);
        }
    }
    ...
    private static final class CostTable {
        ...
        private int get(ChainRange range) {
            return costs[range.start()][range.end()];
        }
