...
public class ModernMergeSort<T extends Comparable<T>> extends ModernBaseSorter<T> {
    private final SortStrategy<T> baseCaseStrategy;

    public ModernMergeSort(SortConfiguration<T> config) {
        this(config, new InsertionSortStrategy<>(config.getComparator(), null));
    }

    public ModernMergeSort(SortConfiguration<T> config, SortStrategy<T> strategy) {
        super(config);
