public final class ModularExponentiation {
    ...
    public static int modPow(ModularExponentiationInput input) {
        ...
        int remainingExponent = input.exponent();
        long result = 1 % input.modulus();

        while (remainingExponent > 0) {
            if ((remainingExponent & 1) != 0) {
                result = multiplyMod(result, normalizedBase, input.modulus());
            }

            normalizedBase = multiplyMod(normalizedBase, normalizedBase, input.modulus());
            remainingExponent >>= 1;
        }
