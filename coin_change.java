...
final class CoinChangeDPService implements CoinChangeStrategy {
    private static final Validator VALIDATOR = CompositeValidator.of(
        CoinChangeDPService::checkNotNull,
        CoinChangeDPService::checkNotEmpty,
        CoinChangeDPService::checkNonNegativeSum,
        CoinChangeDPService::checkPositiveCoins
    );

    private static void checkNotNull(int[] coins, int sum) {
        if (coins == null)
            throw new InvalidCoinChangeInputException("coins must not be null");
    }

    private static void checkNotEmpty(int[] coins, int sum) {
        if (coins.length == 0)
            throw new InvalidCoinChangeInputException("coins must not be empty");
    }

    private static void checkNonNegativeSum(int[] coins, int sum) {
        if (sum < 0)
            throw new InvalidCoinChangeInputException("sum must not be negative");
    }

    private static void checkPositiveCoins(int[] coins, int sum) {
        Arrays.stream(coins)
              .filter(c -> c <= 0)
              .findFirst()
