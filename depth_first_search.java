...
class DfsStrategy<N> implements TraversalStrategy<N> {
    ...
    public void traverse(TraversalContext<N> context) {
        ...
        for (N node : context.nodes()) {
            if (!visited.contains(node)) visit(context, node, visited);
        }
    }

    private void visit(TraversalContext<N> context, N node, Set<N> visited) {
        visited.add(node);
        context.visit(node);
        for (Edge<N> edge : context.getEdges(node)) {
            N neighbor = edge.getTo();
            if (!visited.contains(neighbor)) visit(context, neighbor, visited);
        }
    }
}
