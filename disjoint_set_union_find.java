...
class DefaultDisjointSetFactory implements DisjointSetFactory {
    private final NodeFactory nodeFactory;

    DefaultDisjointSetFactory() {
        this(new DefaultNodeFactory());
    }

    DefaultDisjointSetFactory(NodeFactory nodeFactory) {
        this.nodeFactory = nodeFactory;
    }

    public DisjointSet create(int size) {
        return new UnionFind(size, nodeFactory);
    }
}
...
public class UnionFind implements DisjointSet {
    ...
    public UnionFind(int size, NodeFactory nodeFactory) {
        nodes = new Node[size];
        count = size;
        for (int i = 0; i < size; i++) {
            nodes[i] = nodeFactory.create(i);
        }
    }
    ...
    public static void main(String[] args) {
        int size = 5;
        DisjointSetFactory factory = new DefaultDisjointSetFactory();
        DisjointSet uf = factory.create(size);
        uf.union(1, 2);
        uf.union(3, 4);
