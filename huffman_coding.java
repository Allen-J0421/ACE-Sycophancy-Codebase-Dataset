...
class HuffmanEncoder {
    ...
    private HuffmanNode buildTree(List<Character> chars, Map<Character, Integer> freqMap) {
        ...
        for (int i = 0; i < chars.size(); i++) {
            pq.add(new HuffmanNode(freqMap.get(chars.get(i)), i));
        }
        while (pq.size() >= 2) {
    ...
    private void collectCodes(HuffmanNode node, String prefix, Map<Character, String> codes, List<Character> chars) {
        if (node == null) return;
        if (node.isLeaf()) {
            codes.put(chars.get(node.charIndex), prefix.isEmpty() ? "0" : prefix);
            return;
        }
        collectCodes(node.left, prefix + '0', codes, chars);
        collectCodes(node.right, prefix + '1', codes, chars);
    }
}
