...
class KruskalMST {
    public static void main(String[] args) {
        ...
        MstSolver minKruskal = new KruskalSolver(
            new SolverConfig.Builder(RankedUnionFind::new, new AscendingWeight())
                .observer(new LoggingObserver())
                .build());
        MstSolver filteredKruskal = new KruskalSolver(
            new SolverConfig.Builder(RankedUnionFind::new, new AscendingWeight())
                .observer(new LoggingObserver())
                .edgeFilter(e -> e.weight() > 4)
                .build());
        ...
        runAndPrint("Min spanning tree (weight > 4 filter)", filteredKruskal, graph);
...
record SolverConfig(
    ...
    EdgeSortingStrategy sortingStrategy,
    SolverObserver observer,
    Predicate<Edge> edgeFilter
) {
    static class Builder {
        private final UnionFindFactory ufFactory;
        private final EdgeSortingStrategy sortingStrategy;
        private SolverObserver observer = new NoOpObserver();
        private Predicate<Edge> edgeFilter = e -> true;

        Builder(UnionFindFactory ufFactory, EdgeSortingStrategy sortingStrategy) {
            this.ufFactory = ufFactory;
            this.sortingStrategy = sortingStrategy;
        }

        Builder observer(SolverObserver observer) {
            this.observer = observer;
            return this;
        }

        Builder edgeFilter(Predicate<Edge> edgeFilter) {
            this.edgeFilter = edgeFilter;
            return this;
        }

        SolverConfig build() {
            return new SolverConfig(ufFactory, sortingStrategy, observer, edgeFilter);
        }
    }
}
...
class KruskalSolver implements MstSolver {
    ...
    KruskalSolver(SolverConfig config) {
        this.config = config;
    }
    ...
    public List<Edge> solve(Graph graph) {
        List<Edge> sorted = graph.edges().stream()
            .sorted(config.sortingStrategy().comparator())
            .collect(Collectors.toList());

        UnionFind uf = config.ufFactory().create(graph.vertices());
        List<Edge> mst = new ArrayList<>();

        for (Edge e : sorted) {
            config.observer().onEdgeConsidered(e);
            if (!config.edgeFilter().test(e)) {
                config.observer().onEdgeFiltered(e);
                continue;
            }
            if (uf.find(e.from()) != uf.find(e.to())) {
                uf.union(e.from(), e.to());
                mst.add(e);
                config.observer().onEdgeAccepted(e, mst.size());
                if (mst.size() == graph.vertices() - 1) break;
            } else {
                config.observer().onEdgeRejected(e);
            }
        }

        config.observer().onSolveComplete(mst, mst.stream().mapToInt(Edge::weight).sum());
        return mst;
    }
}
...
class NoOpObserver implements SolverObserver {
    ...
    @Override public void onEdgeFiltered(Edge edge) {}
...
class LoggingObserver implements SolverObserver {
    ...
    @Override
    public void onEdgeFiltered(Edge edge) {
        System.out.println("  Filtered  " + edge + " (predicate)");
    }
