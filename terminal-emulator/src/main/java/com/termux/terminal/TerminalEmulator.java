...
public final class TerminalEmulator {
    ...
    /**
     * Handle the "CSI ... $" rectangular-area sequences: DECCRA (copy), DECSERA/DECERA/DECFRA (selective-erase /
     * erase / fill) and DECCARA/DECRARA (change / reverse attributes). The rectangle coordinates honor origin mode
     * (DECOM) via the effective margins computed below.
     */
    ...
    /** "CSI ... '" - insert (DECIC, '}') or delete (DECDC, '~') columns. */
    private void doCsiSingleQuote(int b) {
    ...
    /** "CSI ? ... $ p" - DECRQM (Request DEC private mode); replies with the mode's current state. */
    ...
    /** "CSI ... SP q" - DECSCUSR (Set Cursor Style); 't'/'u' set margin-bell volume (ignored). */
    private void doCsiArgsSpace(int b) {
        int arg = getArg0(0);
        switch (b) {
    ...
    /** "CSI ... * x" - DECSACE (Select Attribute Change Extent), choosing stream vs rectangular. */
        ...
        switch (b) {
            case (byte) '\\': // End of ESC \ string Terminator
            {
                ...
                if (dcs.startsWith("$q")) {
                    ...
                    // DCS + q P t ST - Request Termcap/Terminfo String.
                    doDeviceControlRequestTermcap(dcs);
                } else {
    ...
    /** DCS $ q P t ST - Request Status String (DECRQSS); only the conformance level (DECSCL) is answered. */
    ...
    /**
     * DCS + q P t ST - Request Termcap/Terminfo String. The string following the "q" is a list of names encoded in
     ...
     */
    private void doDeviceControlRequestTermcap(String dcs) {
        for (String part : dcs.substring(2).split(";")) {
            if (part.length() % 2 == 0) {
                respondToTermcapTerminfoRequest(part);
            } else {
    ...
    /** Decode a single hex-encoded termcap/terminfo name and write back xterm's DCS response for it. */
    ...
    /** Encode a string as upper-case hexadecimal, two digits per character (used for DCS termcap/terminfo responses). */
    private static String hexEncode(String s) {
        StringBuilder hexEncoded = new StringBuilder();
        for (int j = 0; j < s.length(); j++) {
            hexEncoded.append(String.format("%02X", (int) s.charAt(j)));
        }
    ...
    /** "CSI ${0,1,2,3} J" - Erase in Display (ED). Ignores the scrolling margins. */
    ...
    /** "CSI {n} Z" - Cursor Backward Tabulation (CBT). Move the active position n tabs backward. */
    ...
    /** "CSI {n} g" - Clear tab stop (TBC): clear the stop at the cursor (0) or all stops (3). */
    ...
    /** "CSI {n} n" - ECMA-48 Status Report: Device Status Report (5) or Cursor Position Report (6). */
    ...
    /**
     * "CSI {n} T" - with no other parameters, Scroll Down (SD) n lines; with the extended parameter form, initiate
     * highlight mouse tracking (unimplemented).
     */
    ...
    /** "CSI ... t" - Window manipulation (from dtterm, as well as extensions): report state/size, or push/pop title. */
    ...
    /**
     * Apply an extended-color SGR introducer (38 = foreground, 48 = background, 58 = underline). The introducer is
     * followed by either "2;$R;$G;$B" for a 24-bit color or "5;$INDEX" for an indexed color, read from {@link #mArgs}
     * starting just after {@code argIndex}.
     *
     * @param code     the introducer code, one of 38, 48 or 58.
     * @param argIndex index in {@link #mArgs} of the introducer code itself.
     * @return the index of the last argument consumed, so the caller's loop resumes after the color sub-arguments.
     */
    private int applyExtendedColorSgr(int code, int argIndex) {
        if (argIndex + 2 > mArgIndex) return argIndex;
        int firstArg = mArgs[argIndex + 1];
        if (firstArg == 2) {
            if (argIndex + 4 > mArgIndex) {
                Logger.logWarn(mClient, LOG_TAG, "Too few CSI" + code + ";2 RGB arguments");
            } else {
                int red = getArg(argIndex + 2, 0, false);
                int green = getArg(argIndex + 3, 0, false);
                int blue = getArg(argIndex + 4, 0, false);

                if (red < 0 || green < 0 || blue < 0 || red > 255 || green > 255 || blue > 255) {
                    finishSequenceAndLogError("Invalid RGB: " + red + "," + green + "," + blue);
                } else {
                    int argbColor = 0xff_00_00_00 | (red << 16) | (green << 8) | blue;
                    setSgrColor(code, argbColor);
                }
                argIndex += 4; // "2;P_r;P_g;P_r"
            }
        } else if (firstArg == 5) {
            int color = getArg(argIndex + 2, 0, false);
            argIndex += 2; // "5;P_s"
    ...
    /** Store a parsed SGR color into the foreground (38), background (48) or underline (58) slot. */
    ...
    private void doOscSetTextParameters(String bellOrStringTerminator) {
        ...
        switch (value) {
            ...
            case 4:
                ...
                if (!oscChangeColors(textParameter)) return; // Malformed: abort without finishing the sequence.
                break;
    ...
    /**
     * OSC 4: "$c;$spec[;$c;$spec...]" — change indexed color number $c to $spec (an XParseColor name or RGB spec).
     * Any number of index/spec pairs may be given.
     *
     * @return false if the sequence was malformed (the caller should then abort without finishing the sequence).
     */
    private boolean oscChangeColors(String textParameter) {
        int colorIndex = -1;
    ...
    /**
     * OSC 10/11/12: set, or with "?" report, the text foreground (10), background (11) or cursor (12) color. Each
     * successive ;-separated spec advances to the next special color in that list.
     */
    private void oscSetOrReportSpecialColors(int value, String textParameter, String bellOrStringTerminator) {
    ...
    /** OSC 104: "[;$c...]" — reset indexed color $c (or all colors, if no parameter) to the theme default. */
    private void oscResetColors(String textParameter) {
    ...
    /**
     * Translate a code point through the VT100 line-drawing (special graphics) character set used for G0/G1, per
     * http://www.vt100.net/docs/vt102-ug/table5-15.html. Code points without a mapping are returned unchanged.
     */
    private static int translateLineDrawingCodePoint(int codePoint) {
        switch (codePoint) {
            case '_': return ' '; // Blank.
            case '`': return '◆'; // Diamond.
            case '0': return '█'; // Solid block;
            case 'a': return '▒'; // Checker board.
            case 'b': return '␉'; // Horizontal tab.
            case 'c': return '␌'; // Form feed.
            case 'd': return '\r'; // Carriage return.
            case 'e': return '␊'; // Linefeed.
            case 'f': return '°'; // Degree.
            case 'g': return '±'; // Plus-minus.
            case 'h': return '\n'; // Newline.
            case 'i': return '␋'; // Vertical tab.
            case 'j': return '┘'; // Lower right corner.
            case 'k': return '┐'; // Upper right corner.
            case 'l': return '┌'; // Upper left corner.
            case 'm': return '└'; // Left left corner.
            case 'n': return '┼'; // Crossing lines.
            case 'o': return '⎺'; // Horizontal line - scan 1.
            case 'p': return '⎻'; // Horizontal line - scan 3.
            case 'q': return '─'; // Horizontal line - scan 5.
            case 'r': return '⎼'; // Horizontal line - scan 7.
            case 's': return '⎽'; // Horizontal line - scan 9.
            case 't': return '├'; // T facing rightwards.
            case 'u': return '┤'; // T facing leftwards.
            case 'v': return '┴'; // T facing upwards.
            case 'w': return '┬'; // T facing downwards.
            case 'x': return '│'; // Vertical line.
            case 'y': return '≤'; // Less than or equal to.
            case 'z': return '≥'; // Greater than or equal to.
            case '{': return 'π'; // Pi.
            case '|': return '≠'; // Not equal to.
            case '}': return '£'; // UK pound.
            case '~': return '·'; // Centered dot.
